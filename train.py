...
from models.inopnet import get_model
...
def compute_losses(data: Dict, pred_transforms: List, endpoints: Dict,
    ...
    B,_,_ = data['transform_gt'].shape

    # Compute losses
    gt_src_transformed = se3.transform(data['transform_gt'], data['points_src'][..., :3])

    e = torch.Tensor([[0,0,0,1]])[None,:,:].expand(B,-1,-1).to(_device)
    I4 = torch.eye(4)[None,:,:].expand(B,-1,-1).to(_device)
    gt_matrix_inverse = torch.inverse(torch.cat([data['transform_gt'],e], dim=1))
    if loss_type == 'mse':
        ...
        for i in range(num_iter):
            pred_src_transformed = se3.transform(pred_transforms[i], data['points_src'][...,:3])
            pred_matrix = torch.cat([pred_transforms[i],e], dim=1)
            loss2 = torch.matmul(gt_matrix_inverse, pred_matrix)
            loss22 = []
            for j in range(B):  
                loss22.append(torch.norm(loss2[j] - I4[j])**2)
            if reduction.lower() == 'mean':
                losses['mae_{}'.format(i)] = criterion(pred_src_transformed, gt_src_transformed) + torch.mean(torch.stack(loss22))
            elif reduction.lower() == 'none':
                losses['mae_{}'.format(i)] = torch.mean(criterion(pred_src_transformed, gt_src_transformed),
                                                        dim=[-1, -2]) + torch.mean(torch.stack(loss22)) 
    else:
    ...
    for i in range(num_iter):
        refs = []
        srcs = []
        for b in range(len(endpoints['perm_matrices'][0])):
            refs.append((1.0 - torch.sum(endpoints['perm_matrices'][i][b], dim=1)) * _args.wt_inliers)
            srcs.append((1.0 - torch.sum(endpoints['perm_matrices'][i][b], dim=2)) * _args.wt_inliers)
        
        ref_outliers_strength = torch.cat(refs,dim=1)
        src_outliers_strength = torch.cat(srcs,dim=1)
        if reduction.lower() == 'mean':
    ...
    with torch.no_grad():
        ...
        if 'points_src' in data:
            ...
            if endpoints is not None and 'perm_matrices' in endpoints:
                ...
                for i_iter in iters_to_save:
                    ref_weights = torch.sum(endpoints['perm_matrices'][i_iter][0], dim=1)
...
def validate(data_loader, model: torch.nn.Module, summary_writer: SummaryWriter, step: int):
    ...
    summary_metrics = summarize_metrics(all_val_metrics_np)  # mean and rmse
...
def run(train_set, val_set):
    ...
    for epoch in range(0, _args.epochs):
        ...
        for train_data in train_loader:
            ...
            # Compute loss, and optimize, mat is source2ref
            ...
            if global_step % _args.summary_every == 0:  # Save tensorboard logs
                traindata_metric = compute_metrics(train_data,pred_transforms[-1])
                summary_traindata_metric = summarize_metrics(traindata_metric)
                save_summaries(train_writer, data=train_data, predicted=pred_transforms, endpoints=endpoints,
                               losses=train_losses, step=global_step,metrics=summary_traindata_metric)
