...
from common.math import se3 as se3

_logger = logging.getLogger()


def get_train_datasets(args: argparse.Namespace):  # 根据arg参数设置，读取数据
    train_categories, val_categories = None, None  # 训练 和 评估  的类别信息
    if args.train_categoryfile:
        train_categories = [line.rstrip('\n') for line in open(args.train_categoryfile)]  # ./data_loader/modelnet40_half1.txt
        train_categories.sort()
    if args.val_categoryfile:
        val_categories = [line.rstrip('\n') for line in open(args.val_categoryfile)]  # ./data_loader/modelnet40_half1.txt
        val_categories.sort()

    train_transforms, val_transforms = get_transforms(args.noise_type, args.rot_mag, args.trans_mag,
                                                      args.num_points, args.partial)  # 含有随机变换
    ...
    if args.dataset_type == 'modelnet_hdf':
        train_data = ModelNetHdf(args.dataset_path, subset='train', categories=train_categories,  # ../datasets/modelnet40_ply_hdf5_2048
...
def get_transforms(noise_type: str,
    ...
    if noise_type == "clean":
        ...
        train_transforms = [Transforms.Resampler(num_points),  # 重采样-> 1024个点
                            Transforms.SplitSourceRef(),  # 把点云克隆为 源 和 模板点云
                            Transforms.RandomTransformSE3_euler(rot_mag=rot_mag, trans_mag=trans_mag),  # 生成的随机变换
                            Transforms.ShufflePoints()]  # 打乱点云顺序

        test_transforms = [Transforms.SetDeterministic(),  # 使用固定种子
                           Transforms.FixedResampler(num_points),  # 选前1024个点
                           Transforms.SplitSourceRef(),  # # 把点云克隆为 源 和 模板点云
...
class ModelNetHdf(Dataset):  # 读取数据和标签
