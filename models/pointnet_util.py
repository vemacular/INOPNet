...
def sample_and_group_multi(npoint: int, radius: float, nsample: int, xyz: torch.Tensor, normals: torch.Tensor,
    ...
    idx = query_ball_point(radius, nsample, xyz, new_xyz, fps_idx)  # compute neighbor idx based on radius and nsample
    grouped_xyz = index_points(xyz, idx)  # (B, npoint, nsample, C)
    dxyz = grouped_xyz - new_xyz.view(B, S, 1, C)  # d = p_r - p_i  (B, npoint, nsample, 3)
    ni = index_points(normals, idx)  # B npoints nsample 3
    ...
    xyz_feat = dxyz  # (B, npoint, n_sample, 3)
